#!/usr/bin/env python3
"""Collect read-only X research candidates through GetXAPI.

Usage:
  python3 getxapi_research.py search "AI crypto" --limit 10
  python3 getxapi_research.py search "from:VitalikButerin AI" --format markdown
"""
...
def request_json(path: str, query: Optional[dict[str, Any]] = None) -> Any:
    token = api_key()
    if not token:
        raise RuntimeError("Set GETXAPI_API_KEY before using GetXAPI research.")
    ...
    try:
        ...
        raise RuntimeError(f"GetXAPI returned HTTP {exc.code}: {body}") from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"GetXAPI request failed: {exc.reason}") from exc
...
def normalize_tweet(raw: Any, query: str) -> Optional[dict[str, Any]]:
    ...
    username = (
        first_string(raw, ("username", "screen_name", "handle", "author_username", "userName"))
        or first_string(author, ("username", "screen_name", "handle", "name", "userName"))
    ).lstrip("@")
    ...
    return {
        ...
        "likes": first_int(raw, ("like_count", "favorite_count", "likeCount", "likes", "favorites"))
        or first_int(metrics, ("like_count", "favorite_count", "likeCount", "likes", "favorites")),
        "reposts": first_int(raw, ("retweet_count", "repost_count", "retweetCount", "retweets", "reposts"))
        or first_int(metrics, ("retweet_count", "repost_count", "retweetCount", "retweets", "reposts")),
        "replies": first_int(raw, ("reply_count", "replyCount", "replies"))
        or first_int(metrics, ("reply_count", "replyCount", "replies")),
        "quotes": first_int(raw, ("quote_count", "quoteCount", "quotes"))
        or first_int(metrics, ("quote_count", "quoteCount", "quotes")),
        "views": first_int(raw, ("view_count", "viewCount", "impression_count", "views", "impressions"))
        or first_int(metrics, ("view_count", "viewCount", "impression_count", "views", "impressions")),
    }


def search_tweets(query: str, limit: int) -> list[dict[str, Any]]:
    payload = request_json("/twitter/tweet/advanced_search", {"q": query, "limit": limit})
