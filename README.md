...
- Research public X posts through GetXAPI before posting
...
| Env Variable | Description | Where to get |
|---|---|---|
| `GETXAPI_API_KEY` | GetXAPI read/write API key | [getxapi.com](https://www.getxapi.com) |
| `GETXAPI_BASE_URL` | Optional API base URL | Defaults to `https://api.getxapi.com` |
...
```bash
python3 ~/.openclaw/skills/twitter-agent/scripts/getxapi_research.py \
  search "AI crypto" --limit 10 --format markdown
```

Use GetXAPI research output to select topics, reply targets, or quote
